import { getBackendSrv, isFetchError, getDataSourceSrv, } from '@grafana/runtime';
import {
  CoreApp,
  DataQueryRequest,
  DataQueryResponse,
...
import { MyQuery, MyDataSourceOptions, SensorDataResponse, DataSourceResponse, MySecureJsonData, HistoricDataResponse } from './types';
...
const convertPRTGDateTimeToISO = (date_string: string) => {
...
const getRefreshTime = (cache_timeout: number) => {
  return Date.now() - (cache_timeout * 60000)
};

const getAverage = (duration: number) => {
  let avg: number = 0;
...
const getPRTGDate = (unixtime: number) => {
  const dt = new Date(unixtime);
  const str = [
    dt.getFullYear(),
    utils.pad(dt.getMonth(), true),
    utils.pad(dt.getDate(), true),
    utils.pad(dt.getHours(), true),
    utils.pad(dt.getMinutes(), true),
    utils.pad(dt.getSeconds(), true)
  ];
  return str.join("-");
}
    ...
    const promises = options.targets.map(async (target) => {
      ...
      if (target.chart_type === undefined) {
        ...
        if (this.cache[cache_key] && refresh_time < this.cache[cache_key].timestamp) {
          ...
          if ((response.data as HistoricDataResponse).histdata.length > 0)  {
            let channel_name = target.channel_name;
            if (channel_name.includes('Traffic')) {
              channel_name = channel_name + ' (Speed)';
            }
            (response.data as HistoricDataResponse).histdata.forEach(function (x: any) {
        ...
        if (this.cache[cache_key] && refresh_time < this.cache[cache_key].timestamp) {
          ...
          last_value = (response.data as HistoricDataResponse).(sensordata)[SENSOR_DATA_MAPPINGS[target.filter_property]];
